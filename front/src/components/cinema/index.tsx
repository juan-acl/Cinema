...
const Cinema = () => {
  ...
  const reserveSeat = (seat: Seat__) => {
     ...
     */
    let findSeat = seats[0].seats.find((item) =>
      item.seat.find((itemSeat) => itemSeat.no_seat === seat.no_seat),
    );
    ...
    if (containt) {
      ...
      let index = seats[0].seats.findIndex((item) =>
        item.seat.find((ele) => ele.no_seat === seat.no_seat),
      );
  ...
  const saveReservation = async () => {
    ...
    setTimeout(() => {
      Dialog.show({
        ...
        textBody:
          "Tu reservacion se ha realizado con exito, por favor espera a que se muestre tu boleto en pantalla.",
        button: "Ok",
      });
    }, 1000);
  };
      ...
      {isLoading ? (
        ...
        <>
          <View className="items-center justify-center">
            <Text className="text-white text-3xl mt-5 mb-5">
              Elige tus boletos Q75.00 c/u
            </Text>
          </View>
          <View
            style={{
          ...
          <View className="justify-center flex-row">
            <Text className="text-white text-2xl mt-5 mb-5">
              Total a pagar:{" "}
            </Text>
            <Text className="text-white text-2xl mt-5 mb-5">
              Q {totalPay}.00
